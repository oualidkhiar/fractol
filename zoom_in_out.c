...
/*   Updated: 2025/02/20 22:19:03 by oukhiar          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void zoom_in(double mouse_x, double mouse_y, t_mlx_data *mlx)
{
    ...
    mlx->graph.x_start = mouse_x - ((mouse_x - mlx->graph.x_start) * mlx->zoom);
    mlx->graph.x_end = ((mlx->graph.x_end - tmp_x) * mlx->zoom) + mlx->graph.x_start;
    mlx->graph.y_start = mouse_y - ((mouse_y - mlx->graph.y_start) * mlx->zoom);
    mlx->graph.y_end = ((mlx->graph.y_end - tmp_y) * mlx->zoom) + mlx->graph.y_start;
}
...
int mapped_and_zoom(int key_code, int x, int y, t_mlx_data *mlx)
{
    ...
    mapped_x_mouse = ((double)x / WIDTH) * (mlx->graph.x_end - mlx->graph.x_start) + mlx->graph.x_start;
    mapped_y_mouse = (((double)y * (-1)) / HEIGHT) * (mlx->graph.y_end - mlx->graph.y_start) + mlx->graph.y_end;
